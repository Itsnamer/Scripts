	...
	if fruits then
		...
		for _, fruit_data in ipairs(fruits) do
			if table.find(selected_Main_Fruits, fruit_data.Name) then
				print("๐งFound Mained Fruit (Inventory) : ", fruit_data.Name)
		...
		for _, fruit_data in ipairs(fruits) do
			if table.find(selected_Sec_Fruits, fruit_data.Name) then
				print("๐ Found Second Fruit (Inventory) : ", fruit_data.Name)
				return fruit_data.Name
			end
		end
	end
...
function Eat_Select_Fruit(fruit)
	...
	if FindFruitByName(localplayer.Character, equipped_fruit_name) then
		...
		print("๐ฝ๏ธ EatsFruit Completed :", equipped_fruit_name)
	end
...
function Buy_Fruit()
	local function buyFruit(fruit)
		...
		print("โ… BuysFruit Completed :", fruit)
	end
...
localplayer.CharacterAdded:Connect(function(Character)
	...
	humanoid.StateChanged:Connect(function(oldState, newState)
		if newState == Enum.HumanoidStateType.Seated then
			print("๐’บ Humansit is true")

			task.wait(0.1)

			if humanoid.Sit then
				print("โก Humansit is false")
				humanoid.Sit = false
			end
		end
	end)
