return {
	LoadUI = function()
		...
		StatusGui.Text = "Doing Status : Starting Farm"
		...
		task.spawn(function()
			while true do
				...
				local HasPainting = GetsTool("PaintingTool")
				local HasBinding = GetsTool("BindTool")
				local HasProperty = GetsTool("PropertiesTool")
				local HasScaling = GetsTool("ScalingTool")
				local HasTrowel = GetsTool("TrowelTool")
